...
const ANTONYMS = pickRandom(WORD_BANK.antonimos, 4).map((p, i) => ({ id: i + 1, ...p }));

export const Phase8: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  const [cards, setCards] = useState<{ id: string; text: string; pairId: number; isFlipped: boolean; isMatched: boolean }[]>([]);
  const [flippedIds, setFlippedIds] = useState<string[]>([]);
  const [disabled, setDisabled] = useState(false);

  useEffect(() => {
    const initialCards = ANTONYMS.flatMap(p => [
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-2xl mx-auto h-full">
      <div className="text-center mb-8">
        <p className="text-lg text-white/70">Conexão Oposta</p>
        <p className="text-sm text-cyan mt-2">Encontre os pares de palavras que são o OPOSTO uma da outra (Antônimos)!</p>
      </div>
        ...
        {cards.map(card => (
          ...
          >
            ...
            <div className={clsx(
              ...
              card.isMatched ? "bg-green/20 border-green text-green" : "bg-purple-500/20 border-purple-500 text-purple-400"
            )}>
              {card.text}
            </div>
          </button>
